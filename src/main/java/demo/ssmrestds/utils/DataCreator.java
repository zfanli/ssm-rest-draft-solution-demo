...
public class DataCreator {
    public static PaymentItem[] createPaymentItem(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            pi[i].setRegisteredMonth(MessageFormat.format("2019-{0}", i + 1));
            pi[i].setRequestDeadline(MessageFormat.format("2019-1-{0}", i + 1));
            pi[i].setPayStatus(i % 2 == 0 ? "0" : "1");
            pi[i].setMediaName("Test Media Name " + i);
            pi[i].setItemName("Test Item " + i);
            pi[i].setChannel(i % 2 == 0 ? "0" : "1");
            pi[i].setRegisteredStartDate(MessageFormat.format("2019-1-{0}", i + 1));
            pi[i].setRegisteredEndDate(MessageFormat.format("2019-1-{0}", i + 2));
